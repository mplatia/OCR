...
src = cv2.imread(r'IMG.jpg')  # Leer foto
ROI = np.zeros(src.shape, np.uint8)  # Cree una matriz vacía vacía del mismo tamaño que la imagen original para el almacenamientoROIInformación

gray = cv2.cvtColor(src, cv2.COLOR_BGR2GRAY)  # Escala de grises
ret, binary = cv2.threshold(gray, 0, 255,cv2.THRESH_BINARY_INV | cv2.THRESH_TRIANGLE)  # Binarización adaptativa
...
for cnt in range(len(contours)):  # Procesar cada contorno en función de la cantidad de contornos
    ...
    if mm['m00'] != 0:
        ...
        # Analizar geometría
        corners = len(approx)
        if corners == 3 and (color[2] >= 150 or color[0] >= 150) and area > 1000:  # Una serie de condiciones de juicio son ajustadas por las características del proyecto
